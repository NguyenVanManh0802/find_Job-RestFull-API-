...
const ModalUser = (props: IProps) => {
    ...
    const submitUser = async (valuesForm: any) => {
        ...
        const getIdFromSelect = (item: any) => {
            if (!item) return null;
        ...
        const roleId = getIdFromSelect(role);
        const companyId = getIdFromSelect(company);

        // --- SỬA ĐOẠN KHAI BÁO USER NÀY ---
        const user = {
            name,
            email,
            password,
            ...
            gender,
            address,
            // Thêm (as any) để TypeScript không báo lỗi null hoặc thiếu field name
            role: roleId ? { id: roleId } : (null as any),
            company: companyId ? { id: companyId } : (null as any)
        };

        if (dataInit?.id) {
            // Update
            // Thêm (as any) vào đây để ép kiểu dữ liệu gửi đi
            const res = await callUpdateUser({ ...user, id: dataInit.id } as any);
            if (res.data) {
    ...
    async function fetchCompanyList(name: string): Promise<ICompanySelect[]> {
        const res = await callFetchCompany(`page=1&size=100&name ~ '${name}'`); // Fix query filter
        if (res && res.data) {
    ...
    async function fetchRoleList(name: string): Promise<ICompanySelect[]> {
        const res = await callFetchRole(`page=1&size=100&name ~ '${name}'`); // Fix query filter
        if (res && res.data) {
            const list = res.data.result;
            const temp = list.map(item => {
                return {
